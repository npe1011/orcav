...
リポジトリをダウンロード（またはクローン）し、 `uv` を用いて実行してください。
WindowsおよびMac向けにダブルクリックで起動できるスクリプトを用意しています。

* **Windows**: `run_win.bat` をダブルクリックして実行します。
* **Mac**: `run_mac.command` をダブルクリックして実行します。（※初回のみ `chmod +x run_mac.command` で実行権限を付与してください）
